...
const { clearLocal, lightMode, distPath, browserSyncRelativePath } = require('../options');
...
class SLAMBOX {
	...
	async rimrafLocal() {
		...
		return fsPromises.rmdir(path.resolve(__dirname, '..' + distPath), { recursive: true })
	}
	...
	server() {
		...
		browserSync({
			server: browserSyncRelativePath,
			host: 'localhost',
			port: SLAMBOX.port,
			startPath: distPath + "/index.html",
			files: [
				"." + distPath + "components-template/**/style.css",
				"." + distPath + "*.html",
				"." + distPath + "critical.css",
				"." + distPath + "components-template/**/script.min.js",
				"." + distPath + "js/app.min.js",
				"." + distPath + "images/",
			],
			middleware: [
				function (req, res, next) {
					if (lightMode) {
						const file = req.originalUrl.split('/').reverse()[0];
						const matched = file.match(/.*(?=.*\.html)/i);
						if (matched && !req.originalUrl.includes('ajax')) {
							_this.currentPage = matched[0].split('/').join('');
						}
					};
					next();
				},
			...
			bs.io.sockets.on('connection', function (socket) {
				socket.on('build_all', function () {
					if (cooldown) return;
					cooldown = true;
					pugBuilder({
						smpMode,
						callback: () => { cooldown = false }
					});
	...
	styleWatch() {
		...
		(function () {
			const watcher = chokidar.watch('src/components/**/--*.scss', { persistent: true });
			let isReady = false;
			...
			watcher.on('change', path => {
				...
				chunks[chunks.length - 1] = 'style.scss'
	...
	buildMainJs() {
		return buildJs({
			...
			output: path.resolve(__dirname, '..' +distPath+'js/app.min.js'),
			init: true
		});
	}
		...
		return new Promise((resolve, reject) => {
			...
			glob("src/components/**/script.js", function (er, files) {
				files.forEach(src => {
					...
					const output = path.resolve(__dirname, '..' + distPath + 'components-template/', src.replace('src/components/', '').replace('script.js', 'script.min.js'));
					promises.push(
						buildJs({
							input,
							output,
	...
	jsWatch() {
		...
		(function () {
			...
			watcher.on('change', link => {
				...
				const output = path.resolve(__dirname, `..${distPath}components-template/${link.replace('src/components/', '').replace('script.js', 'script.min.js')}`);
				buildJs({
					input,
					output
				})
