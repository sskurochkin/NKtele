window.addEventListener('load', function () {
	...
	window.reinit.slider.productPageSlider = function () {
		...
		if (productPageSlider.length  && !productPageSlider.hasClass('inited')) {
			productPageSlider.each(function (i, el) {
                ...
                const navLength =$(el).closest('.product-page__image').find('.js-slider-product-page-nav').find('.swiper-slide').length
                if (navLength <= 1) {
                    $('.product-image__nav').addClass('hide')
                    return
                }
                ...
                if ($(window).width() > 768 && $('.js-slider-product-page-nav').length) {
                    ...
                    nav = window.slam_slider({
                        ...
                        args: {
                            lazy: true,
                            spaceBetween: 0,
                            threshold: navLength > 4 ? 0 : Infinity,
                            slidesPerView: 4,
                            // noSwiping: navLength <=4,
				...
				let swiper = window.slam_slider({
					...
					args: {
                        ...
                        on: {
                            ...
                            //         let position = 0;
                            //         setTimeout(() => {
                            //             $(el).find('.swiper-wrapper .swiper-slide').each(function (index) {
	...
	$('[data-tab-index]').click(function () {
		...
		if ($(this).attr('data-scroll')) {
			...
			$('html,body').stop().animate({ scrollTop: $(`[data-tab-content=${index}]`).offset().top - $('.js-header-sticky').height() }, 1000);
	...
	$('.checkbox-rate-label').hover(function () {
		...
		if (!$(this).closest('.rate-group').hasClass('touched')) {
			...
			$(`.rate-group input`).prop("checked", false);
		}
		const _this = this;
		let isReach = false
